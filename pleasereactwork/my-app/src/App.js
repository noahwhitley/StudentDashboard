...
import { faUser, faGraduationCap, faEdit, faBell, faCalendarPlus, faTrashAlt, faChevronDown, faChevronUp } from '@fortawesome/free-solid-svg-icons';
...
function App() {
    ...
    const handleAddAlert = () => {
        const newAlert = 'Insert some sort of alert or update to students';
    ...
    const handleRemoveAlert = () => {
        if (alerts.length > 0) {
            ...
            updatedAlerts.pop(); // Remove the last alert
            setAlerts(updatedAlerts);
        }
    };

    const handleAddDeadline = () => {
        const newDeadline = 'Class registration ends 05/31/2024'; // Change this to your desired deadline format
        setRegistrationDeadlines([...registrationDeadlines, newDeadline]);
    };

    const handleRemoveDeadline = () => {
        if (registrationDeadlines.length > 0) {
            const updatedDeadlines = [...registrationDeadlines];
            updatedDeadlines.pop(); // Remove the last deadline
            setRegistrationDeadlines(updatedDeadlines);
        }
    };

    const handleAddExampleClass = () => {
        const newClass = { name: 'Example Class', teacher: 'Example Teacher' };
                ...
                <div className="flex justify-between items-center py-4 px-6 bg-blue-200 w-full h-1/6">
                    ...
                    <div className="flex items-center space-x-4">
                        <ul className="flex items-center gap-[.5vw] text-blue-900 ">
                            <li>
                                <button className="px-4 bg-green-100 rounded-full py-2 hover:bg-green-300" onClick={handleAddAlert}>
                                    <FontAwesomeIcon className="text-green-900 mr-1" icon={faBell} fixedWidth />Add Alert
                                </button>
                            </li>
                            <li>
                                <button className="px-4 bg-red-100 rounded-full py-2 hover:bg-red-300" onClick={handleRemoveAlert}>
                                    <FontAwesomeIcon className="text-red-900 mr-1" icon={faTrashAlt} fixedWidth />Remove Alert
                                </button>
                            </li>
                            <li>
                                <button className="px-4 bg-green-100 rounded-full py-2 hover:bg-green-300" onClick={handleAddDeadline}>
                                    <FontAwesomeIcon className="text-green-900 mr-1" icon={faCalendarPlus} fixedWidth />Add Deadline
                                </button>
                            </li>
                            <li>
                                <button className="px-4 bg-red-100 rounded-full py-2 hover:bg-red-300" onClick={handleRemoveDeadline}>
                                    <FontAwesomeIcon className="text-red-900 mr-1" icon={faTrashAlt} fixedWidth />Remove Deadline
                                </button>
                            </li>
                            <li>
                    ...
                    <div className="w-full mb-8">
                        <div className="p-4 border border-gray-300 bg-white flex items-center justify-between">
                            <div className="ml-4">
                                ...
                                <button className="bg-blue-500 text-white rounded-full mt-4 mr-4 px-4 py-2 hover:bg-blue-600">
                                    Register Now
                                </button>
                                <button className="bg-green-500 text-white rounded-full mt-4 px-4 py-2 hover:bg-green-600" onClick={handleAddExampleClass}>
                                    Insert Example Class
                                </button>
                                <button className="bg-red-500 text-white rounded-full mt-4 ml-4 px-4 py-2 hover:bg-red-600" onClick={handleRemoveClasses}>
                                    Remove Classes
                                </button>
